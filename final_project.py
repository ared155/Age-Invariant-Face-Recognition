        ...
        def main():
            def extract_face(filename, required_size=(224, 224)):
                ...
                if(len(results) > 0):
                    ...
                    err = tk.Label(text="NO FACE DETECTED", background="white", fg="black", font=("", 15))
                    err.place(x=1000, y=400)
                    return None
            ...
            def get_embeddings(filenames):
                ...
                if(len(faces) < 1):
                    flg=1
                    error_str = "No face detected in " + filenames[i]
                    q = tk.Label(text=error_str, background="white", fg="black", font=("", 15))
                    q.place(x=1000, y=400)
                for i in range(len(faces)):
                    if faces[i] is None:
                        flg=1
                        error_str = "NO FACE DETECTED"
                        q = tk.Label(text=error_str, background="white", fg="black", font=("", 15))
                        q.place(x=1000, y=400)
                        # return
                if flg==0:
...
App(tk.Tk(), "Age Invariant Face Recognition", "video.mp4")
